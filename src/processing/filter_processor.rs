...
pub fn apply_filter(oscillator: &WavetableOscillator, current_filter: &Filter) -> BltFilter<WavetableOscillator>{
    ...
    match &current_filter.filter_type {
        ...
        FilterType::LP => filtered_oscillator = oscillator.clone().low_pass(current_filter.filter_cutoff),

        FilterType::HP => filtered_oscillator = oscillator.clone().high_pass(current_filter.filter_cutoff),
